...
async function loadData() {
  try {
    const [calls, clusters, edgesRaw] = await Promise.all([
...
function navigateTo(hash) {
  ...
  if (sectionId === 'network') {
    // Slight delay ensures the DOM has painted the display:block before Sigma reads layout geometry
...
document.querySelectorAll('.nav-btn').forEach(btn => {
  btn.onclick = () => window.location.hash = `#${btn.dataset.section}`;
});
...
function renderNetwork() {
  mountNetworkGraph('react-network-root', tribeEdges, allCalls, clusterSummaries, 0.70);
}
