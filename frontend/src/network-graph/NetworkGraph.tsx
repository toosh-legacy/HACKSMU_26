  ...
  return (
    <div className={`network-graph-root ${className}`} style={{ position: 'relative', width: '100%', height: '100%', background: 'var(--charcoal)' }}>
      ...
      {sigmaSelectedNode && selectedAppNode && (
        <div style={{ position: 'absolute', top: 16, left: '50%', transform: 'translateX(-50%)', zIndex: 20, display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(226,112,58,0.1)', border: '1px solid rgba(226,112,58,0.2)', borderRadius: 12, padding: '8px 16px', backdropFilter: 'blur(4px)' }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--sienna)', animation: 'pulse 2s infinite' }} />
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--linen)' }}>
      ...
      {isLayoutRunning && (
        <div style={{ position: 'absolute', bottom: 16, left: '50%', transform: 'translateX(-50%)', zIndex: 10, display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(122,158,126,0.15)', border: '1px solid rgba(122,158,126,0.25)', borderRadius: 999, padding: '6px 12px', backdropFilter: 'blur(4px)' }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--sage)', animation: 'ping 1s infinite' }} />
          <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--sage)' }}>Layout optimizing...</span>
        </div>
      )}
    </div>
  );
